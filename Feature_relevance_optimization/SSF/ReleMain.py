...
parser.add_argument('--criteria', default = 'dist', help = 'path to save the pruned parameters as checkpoint')
...
def main():
    ...
    if not args.sparse_ratio is None:
        ...
        for ite in range(args.sparse_epoch):
            ...
            sparse_model_path = '{}ByRelevance_SK/{}-{}-epoch{}'.format(args.dataset,
                                                                    args.arch,args.sparse_ratio,args.epochs)
            result_path = '{}ByRelevance_SK/{}-{}-epoch{}/sparse_ite_{}'.format(args.dataset,
                                                                    args.arch,
                                                                    args.sparse_ratio,
                                                                    args.epochs,
                                                                    ite)
            make_path(result_path) 
            sparse_checkpoint_path =  os.path.join(result_path,'similar_sparse_cp.pth.tar')
            if ite ==0: #prune the checkpoint from trained model
                source_checkpoint = args.source_checkpoint
                print('loading normal checkpoint:' + source_checkpoint)
                state_dict = prune_irrelevant(source_checkpoint,NN,testDataloaders,args.sparse_ratio,args.criteria)
                print('prune normal checkpoint to sparse one:' + sparse_checkpoint_path)
                torch.save(state_dict,sparse_checkpoint_path)
            if ite>0:
                ...
                state_dict = prune_irrelevant(source_checkpoint,NN,testDataloaders,args.sparse_ratio,args.criteria)
                torch.save(state_dict,sparse_checkpoint_path) 
            #build model with defined architecture    
...
def write_probability_integrate_result(path):
    ...
    target_label = './ByRelevance/resnet101_pretrained-0.2-epoch300/sparse_ite_0/'
    ...
    result_path = args.dataset+'ByRelevance/'+\
...
def load_data(path):
    ...
    testImageLoaders = torch.utils.data.DataLoader(image_datasets['test'],batch_size=8,shuffle=False)
