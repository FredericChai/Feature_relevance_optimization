...
def filter_prune(load_path,sparse_ratio,arch):
    ...
    if arch.startswith('resnet'):
        # print('begin_to_prune')
        for layer in state_dict:  
            if re.match(r'module.layer+\d+.+\d+.conv+\d+.weight',layer):     
                    ...
                    print("prune {} finished".format(layer))
                    threshold_index = int((1-sparse_ratio)*c*k)
            ...
            elif layer.endswith('module.fc.weight'):
                    one_dim_tensor = torch.abs(state_dict[layer].reshape(-1,)) 
                    threshold_index = int((1-sparse_ratio)*len(one_dim_tensor))
...
def prune(load_path,sparse_ratio,arch):
    ...
    if arch.startswith('resnet'):
        # print('begin_to_prune')
        for k in state_dict:
            if re.match(r'module.layer+\d+.+\d+.conv+\d+.weight',k) or k.endswith('module.fc.weight'):  # The first layer is not the form of module.layer
                #compute the threshold
                # print('prune:',str(k))
                one_dim_tensor = torch.abs(state_dict[k].reshape(-1,)) 
                threshold_index = int(sparse_ratio*len(one_dim_tensor))
...
def hard_prune(current_checkpoint,sparse_ratio,arch,threshold_list):
...
def Random_reduce_prune(load_path,sparse_ratio,arch):
    """prune the weight using random prune"""
    checkpoint = torch.load(load_path)
    state_dict = checkpoint['state_dict']
    ...
    if arch.startswith('vgg16'):
        for k in state_dict:
            if not k.endswith('weight'):
                ...
                #apply threshold to layer
                sparse_tensor = state_dict[k]
                mask = torch.randn(sparse_tensor.shape).cuda()*sparse_tensor
                state_dict = torch.where(torch.abs(sparse_tensor>threshold,sparse_tensor,mask))
    if arch.startswith('resnet101'):
        for k in state_dict:
            if re.match(r'module.layer+\d+.+\d+.conv+\d+.weight',k): 
                #compute the threshold
                one_dim_tensor = torch.abs(state_dict[k].reshape(-1,)) 
                threshold_index = int(sparse_ratio*len(one_dim_tensor))
                sorted_tensor,_ = one_dim_tensor.sort(0,descending = True)
